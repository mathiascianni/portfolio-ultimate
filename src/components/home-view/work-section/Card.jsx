...
//Constants
import { fadeInItem, imageAnimation } from "../../../constants/animations"
...
const Card = ({ card, index }) => {
    ...
    return (
        ...
        >
            <Link to={`/works/${card.id}`} className="">
                <motion.div
                    initial="initial"
                    whileHover={index % 2 === 0 ? "animateR" : "animateL"}
                    href={`/works/${card.id}`}
                    className="group inline-block relative hover:lg:px-6 pb-6 border-primary hover:border-secondary border-b w-full transition-all"
                >
